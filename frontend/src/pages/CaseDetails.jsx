  ...
  return <div className="page-stack">
    ...
    <div className="detail-grid">
      <main className="page-stack">
        <div className="panel">
          <div className="panel-header"><div><span className="eyebrow">VICTIM SUBMISSION</span><h3>Original evidence</h3></div><span className="record-date">{data.capture_mode === 'video' ? 'Video' : data.capture_mode === 'voice' ? 'Audio' : 'Written'}</span></div>
          {(data.narrative || data.transcript) && (
            <div className="submission-text"><span className="submission-label">Statement</span><p>{data.narrative || data.transcript}</p></div>
          )}
          {data.media?.kind === 'video' && (
            <div className="submission-media"><span className="submission-label">Video recording</span><video controls preload="metadata" src={api.getCaseMediaUrl(caseId)} /></div>
          )}
          {data.media?.kind === 'audio' && (
            <div className="submission-media"><span className="submission-label">Audio recording</span><audio controls preload="metadata" src={api.getCaseMediaUrl(caseId)} /></div>
          )}
          {!data.narrative && !data.transcript && !data.media && <p className="muted-copy">No original submission content is available for this case.</p>}
        </div>
